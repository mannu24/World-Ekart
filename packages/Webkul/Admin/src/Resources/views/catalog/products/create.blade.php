                ...
                <div slot="body">
                    ...
                    <div class="control-group" :class="[errors.has('type') ? 'has-error' : '']">
                        ...
                        <select class="control" v-validate="'required'" id="type" name="type"
                            ...
                            <option value="{{ $key }}"
                                {{ request()->input('type') == $productType['key'] ? 'selected' : '' }}>
                    ...
                    <div class="control-group" :class="[errors.has('attribute_family_id') ? 'has-error' : '']">
                        ...
                        <select class="control" v-validate="'required'" id="attribute_family_id"
                            name="attribute_family_id" {{ $familyId ? 'disabled' : '' }}
                            data-vv-as="&quot;{{ __('admin::app.catalog.products.familiy') }}&quot;">
                            @foreach ($families as $family)
                            <option value="{{ $family->id }}"
                                {{ ($familyId == $family->id || old('attribute_family_id') == $family->id) ? 'selected' : '' }}>
                                {{ $family->name }}</option>
                            @endforeach
                        </select>
                        ...
                        <span class="control-error"
                            v-if="errors.has('attribute_family_id')">@{{ errors.first('attribute_family_id') }}</span>
                    </div>
                    ...
                    <div class="control-group" :class="[errors.has('sku') ? 'has-error' : '']">
                        ...
                        <input type="text" v-validate="{ required: true, regex: /^[a-zA-Z0-9]+(?:-[a-zA-Z0-9]+)*$/ }"
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <span class="label">
                                            <input type="hidden" name="super_attributes[{{$attribute->code}}][]"
                                                value="{{ $option->id }}" />
                                            {{ $option->admin_name }}
